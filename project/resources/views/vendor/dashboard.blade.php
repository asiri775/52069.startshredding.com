...
<div class="row">
	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 left-table medium-right stick-able">
		<div class="bg-yellow">
			<div class="sale-summary">
				<form method="get" action="" id="filterModel">
					<select onchange="document.getElementById('filterModel').submit();" class="form-control" name="time">
						<option <?php if(isset($_GET['time'])){ if($_GET['time']=="today"){ echo 'selected=""'; }}?> value="today">Today</option>
						<option <?php if(isset($_GET['time'])){ if($_GET['time']=="week"){ echo 'selected=""'; }}?> value="week">This Week</option>
						<option <?php if(isset($_GET['time'])){ if($_GET['time']=="month"){ echo 'selected=""'; }}?> value="month">This Month</option>
						<option <?php if(isset($_GET['time'])){ if($_GET['time']=="year"){ echo 'selected=""'; }}?> value="year">This Year</option> 
						<option <?php if(isset($_GET['time'])){ if($_GET['time']=="all"){ echo 'selected=""'; }}?> value="all">All Time</option>
					</select>  
				</form>
				...
				<div class="order-list">
					<ul>
						<li><a href="#">Pending (<?=$status['pending']?>)</a></li>
						<li><a href="#">In Transit (<?=$status['in_transit']?>)</a></li>
						<li><a href="#">At Plant - Received (<?=$status['at_plan_rece']?>)</a></li>
						<li><a href="#">At Plant - Completed (<?=$status['at_plan_com']?>)</a></li>
						<li><a href="#">On Delivery (<?=$status['on_deliver']?>)</a></li>
						<li><a href="#">Completed - Delivery (<?=$status['completed_deliver']?>)</a></li>
						<li><a href="#">Completed - in Store (<?=$status['completed_in_store']?>)</a></li>
					</ul>
				</div>
			</div>
		</div>
